...
app.get('/', function (req, res) {
  ...
  BuySpare.find(
    ...
    '_provider date noBill serie noDoc details discount total'
  )
    .populate('_provider', 'name')
    .populate('details._part')
    .sort({ date: 'desc' })
    .exec(function (err, buySpares) {
      if (err) {
    ...
    .then(function (buySpareG) {
      var promises = buySpareG.details.map(function (e) {
        AutoCellar.findOne(
          ...
          AutoCellar.updateOne(
            { 'storage._autopart': e._part },
            { 'storage.$.stock': newStock, 'storage.$.cost': e.cost },
