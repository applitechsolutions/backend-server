...
app.put('/purchase/', mdAuth.verificaToken, function(req, res) {
    ...
    MaterialCellar.findOne({ 'storage._material': body._material }, { 'storage.$': 1 })
        .exec(function(err, cellar) {
            ...
            MaterialCellar.updateOne({ 'storage._material': body._material }, { 'storage.$.stock': newStock }, function(err, storageAct) {
                if (err) {
                    res.status(400).json({
                        ok: false,
    ...
    material.save()
        .then(function(materialG) {
            MaterialCellar.findOne({ name: 'Centro de distribución' })
                .then(function(cellar) {
                    var material = { _material: materialG._id, stock: 0, cost: 0.00 };
